<section id="resources" class="section">
    <div class="wrap">
        <div class="inset-blue">
            <div class="wrap-inset md:items-center" style="--cols: 1fr 1fr">
                <h2 class="title-xl">
                    Popular <br>Resources
                </h2>
                <ul class="text-2xl links-black links-underline bullets bullets-blue">
                        <li><span class="icon">{{ svg('icons/far-angle-right') }}</span> <a href="{{ route('open-source.packages') }}">Search all packages</a></li>
                        <li><span class="icon">{{ svg('icons/far-angle-right') }}</span> <a href="https://docs.irabu.dev">All documentation</a></li>
                        <li><span class="icon">{{ svg('icons/far-angle-right') }}</span> <a href="https://github.com/irabu-dev">GitHub</a></li>
                    </ul>
            </div>
        </div>
    </div>
</section>
